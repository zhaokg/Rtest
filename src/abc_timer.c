...
#if ( defined(COMPILER_CLANG)||defined(COMPILER_GCC)||defined(COMPILER_SOLARIS) ) && ! defined(OS_MAC)
...
// stackoverflow.com/questions/9887839/how-to-count-clock-cycles-with-rdtsc-in-gcc-x86
...
#elif defined(cpu_ARM64) && defined(COMPILER_CLANG)
    ...
    */
      ...
      // Note: __builtin_readcyclecounter is primarily available in Clang/LLVM and  
	  // not available in GCC on Linux aarch64(arm64) due to the missing symbol errors when linked
	  // for the R package on Linux aarch64.
 
#elif defined(cpu_ARM64) && defined(COMPILER_GCC)
